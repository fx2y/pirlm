...
def build_incident_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pirml incident", description="Spec-10 incident one-command bundle"
    )
...
def _class_name(*, fail_tag: str, final_ok: bool, replay_match: bool, artifact_parity: bool) -> str:
    if fail_tag:
        return fail_tag if fail_tag in FAIL_TAGS else f"UNKNOWN_{fail_tag}"
...
def run_incident_command(argv: list[str]) -> int:
