...
def build_surface_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pirml surface", description="Spec-10 surface resolver views"
    )
...
def _console_view(run_dir: Path) -> dict[str, Any]:
    ...
    if frames and isinstance(frames[-1].get("meta"), dict):
        meta_obj = cast(dict[str, Any], frames[-1]["meta"])
...
def run_surface_command(argv: list[str]) -> int:
