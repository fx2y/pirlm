    ...
    if event_type == "tool_execution_start":
        ...
        if tool == "pirml_run":
            ...
            if not prog or not out_dir:
                return {
                    "type": "pirml_error",
                    "error": {"type": "integrity", "msg": "Missing prog or out-dir in tool args", "retryable": False}
                }
...
def run_headless(stream: Any = sys.stdin, project_root: Path = Path(".")) -> None:
    ...
    if os.environ.get("PIRML_ENABLE_JSON_HEADLESS") != "1":
        err = UnsupportedError("JSON headless mode disabled. Set PIRML_ENABLE_JSON_HEADLESS=1 to enable.")
