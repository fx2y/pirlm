...
from .protocol import JSONObject, normalize_frames
...
def _new_call_id(index: int) -> str:
...
def run_live(
    ...
    for idx, step in enumerate(program, start=1):
        ...
        row: JSONObject = {"id": call_id, "tool": str(tool), "ok": bool(result_frame["ok"])}
    ...
    final_result: JSONObject = {"ok": ok, "results": result_rows}
...
def run_replay(frames: list[JSONObject], max_line_bytes: int) -> RunOutput:
    ...
    return RunOutput(frames=normalized_frames, final_result=cast(JSONObject, final_raw))
