...
## Hard Laws
- `H0` Root authority is this file; path policy only via imported `.codex/rules/*`.
...
- `H8` Evidence law: every run (incl fatal) emits `trace.ndjson` + compact `final.json`; all emitted pointers resolve.
...
- `H11` Fail-closed: unknown `op|tool|schema|plan|provider|variant|path|flag|cmd` => typed `{type,msg,retryable}`; no fallback/skip/swallow.
...
- `H15` Ingress explicit-only: artifact/schema/report/dataset inputs must be declared args; no implicit discovery.
- `H16` Replay outranks live: parity drift invalidates live output; replay guard runs real deterministic snapshots.
...
- `H20` Status truth: current-cycle `spec-0/*-tasks.jsonl` only; htn/tutorial/learnings are support, not authority.
...
- `H26` Dataset integrity: scored prompt key never falls back to answer fields; duplicate `task_id` is integrity-fail.
...
- `H29` Metrics/taxonomy law: required metric keys fixed by schema; `fail_tag` single-label; `NO_CITE` explicit; persisted `acc` exact.
- `H30` Helper gates additive-only; `ci`/`fast` byte contracts cannot widen silently; fixture helpers labeled smoke.
- `H31` `done` requires contradictions resolved (`owner+enforce`), matrix refs resolvable, live locus exists, named tests pass, proof cmds rerun clean.
...
- `H33` Command-matrix law: one authoritative matrix source; authority rows must use approved owner paths; aliases are non-authoritative hints only.
- `H34` Proof-pack law: canonical lanes emit deterministic index rows `{lane,rc,sha256,trace_ptr,final_ptr,...}`; success rows require resolvable pointers.
- `H35` Incident law: fixed chain `trace->classify->replay(blocked-tools)->artifact-parity`; mismatch/corruption hard-fails typed `integrity` (`rc=2`).
- `H36` Resolver law: operator surfaces are read-only projections over existing artifacts; no runtime/protocol/persistence mutation.
- `H37` Claim law: external/demo/sales claims must bind `{invariant_ref,proof_cmd,artifact_ptr}`; unresolved binds fail-closed.
...
- `D7` Unimplemented seam returns typed unsupported; no dead knobs.
