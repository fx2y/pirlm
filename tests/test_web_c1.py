...
class WebC1Tests(unittest.IsolatedAsyncioTestCase):
    async def test_cached_fetch_304(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            results = [f for f in frames if cast(Any, f).get("op") == "fetch_result"]
            self.assertTrue(any(cast(Any, f).get("cache_hit") for f in results))
    ...
    async def test_sha256_deduplication(self) -> None:
        ...
        self.assertEqual(cast(Any, frames[0]).get("op"), "search_call")
        self.assertEqual(cast(Any, frames[1]).get("op"), "search_result")
        self.assertEqual(cast(Any, frames[1]).get("ms"), 10)
