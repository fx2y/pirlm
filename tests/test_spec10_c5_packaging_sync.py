...
class TestSpec10C5PackagingSync(unittest.TestCase):
    ...
    def _fixture_pack_rows(self) -> tuple[list[dict[str, object]], list[Path]]:
        rows: list[dict[str, object]] = [{"k": "meta", "id": "spec10-proof-pack"}]
        paths: list[Path] = []
        for lane in [f"W{i}" for i in range(11)]:
            ptr = Path(f"out/spec10_pack/{lane.lower()}/final.json")
            paths.append(ptr)
    ...
    def test_every_claim_maps_to_proof_row(self):
        with TemporaryDirectory(prefix="spec10_c5_claims_") as tmp:
            ...
            pack_rows, paths = self._fixture_pack_rows()
            ...
            for line in (out_dir / "persona_pack.jsonl").read_text(encoding="utf-8").splitlines():
                ...
                if row.get("k") == "persona":
                    ...
                    self.assertTrue(row.get("proof_cmd"))
                    self.assertTrue(row.get("artifact_ptr"))
            self.assertGreaterEqual(len(persona_rows), 10)

    def test_claim_without_pointer_fails(self):
        matrix_rows = self._fixture_matrix_rows()
        pack_rows, _ = self._fixture_pack_rows()
        # Break required lane pointer.
        pack_rows = [
        ...
        with self.assertRaises(CliFailure) as ctx:
            spec10_sales_pack.build_persona_pack(
                ...
                pack_index_path=Path("out/spec10_pack/index.jsonl"),
            )
    ...
    def test_priority_lane_order(self):
        rows = spec10_sales_pack.build_persona_pack(
            matrix_rows=self._fixture_matrix_rows(),
            pack_rows=self._fixture_pack_rows()[0],
    ...
    def test_objection_rows_reference_invariants(self):
        rows = spec10_sales_pack.build_persona_pack(
            matrix_rows=self._fixture_matrix_rows(),
            pack_rows=self._fixture_pack_rows()[0],
            verification_rows=self._fixture_verification_rows(),
    ...
    def test_optional_lanes_labeled_unsupported(self):
        rows = spec10_sales_pack.build_persona_pack(
            matrix_rows=self._fixture_matrix_rows(),
            pack_rows=self._fixture_pack_rows()[0],
            verification_rows=self._fixture_verification_rows(),
    ...
    def test_disqualifier_row_missing_fails(self):
        rows = spec10_sales_pack.build_persona_pack(
            matrix_rows=self._fixture_matrix_rows(),
            pack_rows=self._fixture_pack_rows()[0],
            verification_rows=self._fixture_verification_rows(),
    ...
    def test_doc_rows_reference_matrix_ids(self):
