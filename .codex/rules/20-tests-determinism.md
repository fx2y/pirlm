...
- `T12` CLI parse tests required: type/missing/invalid-scalar parse failures emit typed stderr envelope + correct `rc`; no raw `usage:` dumps.
...
- `T17` Pointer/projection tests required: resolvability + non-destructive projection + payload split (`custom.data/details` vs one-line hint `<=120`).
